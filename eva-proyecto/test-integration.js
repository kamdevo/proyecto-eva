...
async function testEndpoints() {
  ...
  try {
    ...
    const medicalDevices = await fetchAPI(
      "/equipos/medical-devices-complete?page=1&per_page=5"
    );
    console.log("✅ Respuesta exitosa:");
    console.log(`   - Total de equipos: ${medicalDevices.total || "N/A"}`);
    console.log(`   - Página actual: ${medicalDevices.current_page || "N/A"}`);
    console.log(
      `   - Equipos en esta página: ${medicalDevices.data?.length || 0}`
    );
    ...
    console.log("✅ Respuesta exitosa:");
    console.log(
      `   - Servicios disponibles: ${filterOptions.servicios?.length || 0}`
    );
    console.log(`   - Áreas disponibles: ${filterOptions.areas?.length || 0}`);
    console.log(
      `   - Estados disponibles: ${filterOptions.estados?.length || 0}`
    );
    console.log("");
    ...
    if (medicalDevices.data && medicalDevices.data.length > 0) {
      ...
      console.log(
        `4️⃣ Probando endpoint: /equipos/${firstDeviceId}/complete-info`
      );
      const deviceInfo = await fetchAPI(
        `/equipos/${firstDeviceId}/complete-info`
      );
      console.log("✅ Respuesta exitosa:");
    ...
    console.log(
      "   1. Asegúrate de que el backend esté ejecutándose en localhost:8000"
    );
    console.log("   2. Verifica que la base de datos esté conectada");
    console.log(
      "   3. Confirma que las rutas estén configuradas correctamente"
    );
...
function validateResponseStructure(data, expectedFields) {
  ...
  if (missingFields.length > 0) {
    console.warn(
      `⚠️  Campos faltantes en la respuesta: ${missingFields.join(", ")}`
    );
    return false;
  }
