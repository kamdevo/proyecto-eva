...
tests.forEach(test => {
    ...
    console.log(`${isValid ? '✅' : '❌'} ${test.field}: ${test.value} ${isValid ? '(correcto)' : '(error)'}`);
});
...
potentialObjects.forEach(obj => {
    ...
    console.log(`    Tipo: ${obj.type} ${isString ? '(seguro para React)' : '(⚠️ revisar)'}`);
});
...
const fieldMapping = [
    ...
    { old: 'device.ultimo_mantenimiento', new: 'device.mantenimiento?.ultimoMantenimiento' },
    { old: 'device.ultima_calibracion', new: 'device.mantenimiento?.ultimaCalibración' },
    { old: 'device.ultimo_correctivo', new: 'device.mantenimiento?.ultimoCorrectivo' },
    ...
    { old: 'device.fecha_inicio_ultimo_ticket', new: 'device.tickets?.fechaUltimoTicket' },
    { old: 'device.cuenta_archivos', new: 'device.data?.archivos' },
    { old: 'device.cuenta_planes_mantenimientos', new: 'device.data?.planesMantenimiento' },
