...
async function runAllTests() {
  ...
  if (passed === total) {
    ...
    log(
      "\n✅ La integración de equipos médicos está funcionando correctamente",
      "green"
    );
    ...
    if (passed >= total * 0.75) {
      ...
      log(
        "   Revisa los detalles arriba para optimizaciones menores",
        "yellow"
      );
    } else {
  ...
  log(
    "2. Si hay errores de autenticación: Asegúrate de tener una sesión válida",
    "blue"
  );
  log(
    "3. Si hay errores de conectividad: Verifica que el backend esté ejecutándose",
    "blue"
  );
  log(
    "4. Para pruebas completas: Usa la interfaz web con usuario autenticado",
    "blue"
  );
