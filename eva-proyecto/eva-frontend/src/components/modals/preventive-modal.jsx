...
const preventiveData = [
  {
    ...
    modelo: "MTC-200",
    estado: "RADIOTERAPIA",
    ubicacion: "SERVICIO CANCER",
    codigo2: "PREVENTIVO",
    ubicacion2: "SERVICIO CANCER",
    acciones: "🔍",
  },
  // Add more data as needed
]

export function PreventiveModal({ open, onOpenChange }) {
  return (
                ...
                {preventiveData.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="border border-gray-300 p-2">
                      <Badge className="bg-green-100 text-green-800">{item.estado}</Badge>
                    </td>
                    <td className="border border-gray-300 p-2">{item.ubicacion}</td>
                    <td className="border border-gray-300 p-2">
                      <Badge className="bg-blue-100 text-blue-800">{item.codigo2}</Badge>
                    </td>
