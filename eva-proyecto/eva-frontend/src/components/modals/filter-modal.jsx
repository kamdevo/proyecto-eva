...
export function FilterModal({ open, onOpenChange, onFiltersApply, onFiltersClear, currentFilters = {} }) {
  ...
  const handleApplyFilters = () => {
    ...
    toast.success(`Filtros aplicados: ${Object.keys(activeFilters).length} criterios`)
    onOpenChange(false)
  }
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await axios.post('/api/v1/equipos/export', activeFilters, {
        responseType: 'blob'
      })
      ...
      link.href = url
      link.download = `equipos_filtrados_${new Date().toISOString().split('T')[0]}.xlsx`
  ...
  const filteredServicios = filterOptions.servicios.filter(servicio =>
    !filters.filtro_zona || filters.filtro_zona === 'all' || servicio.sede_id === parseInt(filters.filtro_zona)
  )

  // Filtrar áreas por servicio seleccionado
  const filteredAreas = filterOptions.areas.filter(area =>
    !filters.servicio_id_auxiliar || filters.servicio_id_auxiliar === 'all' || area.servicio_id === parseInt(filters.servicio_id_auxiliar)
  )

  // Contar filtros activos
  const activeFiltersCount = Object.values(filters).filter(value =>
    value !== '' && value !== null && value !== undefined && value !== 'all' && value !== new Date().getFullYear()
  ).length
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-7xl min-w-5xl w-[95vw] max-h-[95vh] overflow-y-auto">
        ...
        <div className="space-y-6 p-2 sm:p-3 md:p-4">
          ...
          <div className="flex justify-between items-center mb-6 p-4 bg-slate-50 rounded-lg">
            ...
            <div className="text-sm text-gray-600">
              {activeFiltersCount > 0 ? `${activeFiltersCount} filtros activos` : 'Sin filtros aplicados'}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleFilterChange('filtro_code', e.target.value)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleFilterChange('filtro_name', e.target.value)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleFilterChange('filtro_serial', e.target.value)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleFilterChange('filtro_marca', e.target.value)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleFilterChange('filtro_modelo', e.target.value)}
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <Select value={filters.filtro_zona} onValueChange={(value) => handleFilterChange('filtro_zona', value)}>
              ...
              <div>
                <Label htmlFor="servicio_id_auxiliar" className="text-sm font-medium">
                  Servicio Médico:
                </Label>
                <Select
                  value={filters.servicio_id_auxiliar}
                  onValueChange={(value) => handleFilterChange('servicio_id_auxiliar', value)}
                  disabled={!filters.filtro_zona || filters.filtro_zona === 'all'}
                >
                  <SelectTrigger className="mt-1">
              ...
              <div>
                <Label htmlFor="area_id_auxiliar" className="text-sm font-medium">
                  Área Específica:
                </Label>
                <Select
                  value={filters.area_id_auxiliar}
                  onValueChange={(value) => handleFilterChange('area_id_auxiliar', value)}
                  disabled={!filters.servicio_id_auxiliar || filters.servicio_id_auxiliar === 'all'}
                >
                  <SelectTrigger className="mt-1">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <Label htmlFor="filtro_estadoequipo_id" className="text-sm font-medium">
                  Estado del Equipo:
                </Label>
                <Select value={filters.filtro_estadoequipo_id} onValueChange={(value) => handleFilterChange('filtro_estadoequipo_id', value)}>
                  <SelectTrigger className="mt-1">
              ...
              <div>
                ...
                <Select value={filters.filtro_estadom} onValueChange={(value) => handleFilterChange('filtro_estadom', value)}>
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder="Seleccionar estado..." />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="proveedor_mantenimiento" className="text-sm font-medium">
                  Proveedor de Mantenimiento:
                </Label>
                <Select value={filters.proveedor_mantenimiento} onValueChange={(value) => handleFilterChange('proveedor_mantenimiento', value)}>
                  <SelectTrigger className="mt-1">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <Select value={filters.tipo_id} onValueChange={(value) => handleFilterChange('tipo_id', value)}>
                  <SelectTrigger className="mt-1">
              ...
              <div>
                ...
                <Select value={filters.estado_id} onValueChange={(value) => handleFilterChange('estado_id', value)}>
                  <SelectTrigger className="mt-1">
              ...
              <div>
                ...
                <Select value={filters.estado_id_cg} onValueChange={(value) => handleFilterChange('estado_id_cg', value)}>
                  <SelectTrigger className="mt-1">
              ...
              <div>
                ...
                <Select value={filters.anio_plan.toString()} onValueChange={(value) => handleFilterChange('anio_plan', parseInt(value))}>
                  <SelectTrigger className="mt-1">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(filters).map(([key, value]) => {
                  if (value !== '' && value !== null && value !== undefined && value !== 'all' && value !== new Date().getFullYear()) {
                    return (
                      <Badge key={key} variant="secondary" className="text-xs">
                        {key.replace('filtro_', '').replace('_', ' ')}: {value}
                        <X
                          className="h-3 w-3 ml-1 cursor-pointer"
                          onClick={() => handleFilterChange(key, key.includes('id') ? 'all' : '')}
                        />
                      </Badge>
                    )
                  }
        ...
        <div className="flex justify-between items-center p-4 border-t bg-gray-50">
          <div className="text-sm text-gray-600">
            {activeFiltersCount > 0 ? `${activeFiltersCount} filtros configurados` : 'Configure los filtros deseados'}
          </div>
