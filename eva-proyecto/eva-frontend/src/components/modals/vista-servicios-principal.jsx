...
// Importar modales
import UIModalAgregarServicio from "./ui-modal-agregar-servicio"
import UIModalEditarServicio from "./ui-modal-editar-servicio"
import UIModalEliminarServicio from "./ui-modal-eliminar-servicio"
...
export default function VistaServiciosPrincipal() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader className="bg-slate-100">
                  <TableRow>
                    <TableHead className="font-semibold text-slate-700 min-w-[200px]">Nombre</TableHead>
                    <TableHead className="font-semibold text-slate-700 min-w-[150px]">Zona</TableHead>
                    <TableHead className="font-semibold text-slate-700 min-w-[200px]">Centro de costo</TableHead>
                    <TableHead className="font-semibold text-slate-700 min-w-[120px]">Sede</TableHead>
                    ...
                    <TableHead className="font-semibold text-slate-700 text-center min-w-[100px]">Acciones</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {serviciosData.map((servicio) => (
                    <TableRow key={servicio.id} className="hover:bg-gray-50">
                      <TableCell className="font-medium text-sm">{servicio.nombre}</TableCell>
                      <TableCell className="text-sm">{servicio.zona}</TableCell>
                      <TableCell className="text-sm">{servicio.centroCosto}</TableCell>
                      <TableCell className="text-sm">
                        <Badge variant="outline" className="text-xs">
                          {servicio.sede}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-center text-sm font-medium">{servicio.equiposAsociados}</TableCell>
                      <TableCell className="text-center text-sm font-medium">{servicio.areasAsociadas}</TableCell>
            ...
            <div className="p-6 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-sm text-gray-600">
                  Mostrando 1 a {Math.min(itemsPerPage, totalItems)} de {totalItems} entradas
                </div>
      ...
      {/* Modales */}
      <UIModalAgregarServicio isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} />
