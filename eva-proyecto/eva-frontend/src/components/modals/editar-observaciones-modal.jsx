...
export function EditarObservacionesModal({ open, onOpenChange, equipo }) {
  ...
  const handleSave = (id) => {
    setObservaciones((prev) =>
      prev.map(
        (obs) => (obs.id === id ? { ...obs, texto: editText, prioridad: editPrioridad } : obs)
      ))
    setEditingId(null)
    setEditText("")
    setEditPrioridad("")
      ...
      <DialogContent className="w-[95vw] max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <DialogHeader className="border-b border-green-200 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <DialogTitle className="text-xl font-semibold text-slate-800">Editar Observaciones</DialogTitle>
            </div>
            <Button
              variant="ghost"
              size="sm"
        ...
        <div className="py-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium text-slate-800">Observaciones Registradas</h3>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <Badge className={getPrioridadColor(observacion.prioridad)}>
                        ...
                        {editingId === observacion.id ? (
                          ...
                          <>
                            <p className="text-sm text-slate-700 mb-2">{observacion.texto}</p>
                            <div className="text-xs text-slate-500">
                              <span className="font-medium">Responsable:</span> {observacion.responsable}
                            </div>
                          </>
                        )}
                      </div>
