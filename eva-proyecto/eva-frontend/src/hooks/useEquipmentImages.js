...
export const useEquipmentImages = () => {
   ...
   */
  const getEquipmentImage = useCallback(async (equipmentId) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/v1/equipos/${equipmentId}/files`, {
        headers: {
      ...
      if (response.data.success && response.data.data.imagen) {
        ...
        const imageUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/storage/${imageData.path}`;
      ...
      console.error(`Error loading image for equipment ${equipmentId}:`, error);
   ...
   */
  const preloadImages = useCallback(async (equipmentIds) => {
   ...
   */
  const getCachedImage = useCallback((equipmentId) => {
    return imageCache.get(equipmentId);
  }, [imageCache]);
   ...
   */
  const isImageLoading = useCallback((equipmentId) => {
