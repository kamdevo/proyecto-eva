...
class ConnectionPool {
  ...
  async createInitialConnections() {
    ...
    for (const endpoint of this.config.endpoints) {
      ...
      const connectionsPerEndpoint = Math.ceil(
        this.config.minPoolSize / this.config.endpoints.length
      );
  ...
  getRoundRobinConnection() {
    const healthyConnections = Array.from(this.connections.values()).filter(
      (conn) => conn.state === CONNECTION_STATES.HEALTHY && conn.isIdle
    );

    if (healthyConnections.length === 0) return null;

    const connection =
      healthyConnections[this.roundRobinIndex % healthyConnections.length];
  ...
  getLeastConnectionsConnection() {
    ...
    for (const connection of this.connections.values()) {
      if (connection.state === CONNECTION_STATES.HEALTHY) {
        ...
        endpointConnections.set(
          connection.endpoint,
          count + (connection.isIdle ? 0 : 1)
        );
      }
    }
  ...
  getFastestResponseConnection() {
    ...
    for (const [endpoint, stats] of this.endpointStats.entries()) {
      if (
        stats.state === CONNECTION_STATES.HEALTHY &&
        stats.responseTime < fastestTime
      ) {
  ...
  async testConnection(endpoint, signal) {
    try {
      const response = await fetch(
        `${endpoint.url}${this.config.healthCheckPath}`,
        {
          method: "GET",
          signal,
  ...
  async performHealthChecks() {
    ...
    logger.debug(LOG_CATEGORIES.NETWORK, "Health checks completed", {
      healthyEndpoints: Array.from(this.endpointStats.values()).filter(
        (stats) => stats.state === CONNECTION_STATES.HEALTHY
      ).length,
  ...
  async createConnectionFromHealthyEndpoint() {
    ...
    const selectedEndpoint = healthyEndpoints.reduce((min, endpoint) => {
      ...
      return endpointStats.activeConnections < minStats.activeConnections
        ? endpoint
        : min;
    });
   ...
   */
  getHealthStatus() {
    const healthyEndpoints = Array.from(this.endpointStats.values()).filter(
      (stats) => stats.state === CONNECTION_STATES.HEALTHY
    ).length;
