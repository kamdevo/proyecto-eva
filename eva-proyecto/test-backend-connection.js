...
async function testBackendConnection() {
  ...
  try {
    ...
    const medicalDevicesResponse = await fetch(`${API_BASE_URL}/equipos/medical-devices-complete?page=1&per_page=5`);
    
    if (!medicalDevicesResponse.ok) {
      const errorText = await medicalDevicesResponse.text();
      throw new Error(`HTTP ${medicalDevicesResponse.status}: ${medicalDevicesResponse.statusText}\nRespuesta: ${errorText}`);
    }
    ...
    console.log('   - Total equipos:', medicalDevicesData.total || medicalDevicesData.data?.length || 'N/A');
    console.log('   - Página actual:', medicalDevicesData.current_page || 'N/A');
    console.log('');
    
    // 3. Probar endpoint de opciones de filtros
    console.log('3️⃣ Probando endpoint: /equipos/filter-options');
    const filterOptionsResponse = await fetch(`${API_BASE_URL}/equipos/filter-options`);
    
    if (!filterOptionsResponse.ok) {
      const errorText = await filterOptionsResponse.text();
      throw new Error(`HTTP ${filterOptionsResponse.status}: ${filterOptionsResponse.statusText}\nRespuesta: ${errorText}`);
    }
    ...
    console.log('   - Servicios disponibles:', filterOptionsData.servicios?.length || 'N/A');
    console.log('   - Áreas disponibles:', filterOptionsData.areas?.length || 'N/A');
    console.log('');
    
    console.log('🎉 ¡Todas las pruebas de backend pasaron exitosamente!');
    console.log('✅ El backend está funcionando correctamente en:', API_BASE_URL);
    
  } catch (error) {
    console.error('❌ Error de conectividad del backend:', error.message);
    console.log('\n🔧 Verificaciones recomendadas:');
    console.log('   1. Asegúrate de que el servidor Laravel esté ejecutándose:');
    console.log('      cd eva-backend && php artisan serve');
    console.log('   2. Verifica que el servidor esté en: http://localhost:8000');
